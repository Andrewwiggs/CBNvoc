#加载ImageNet预训练权重，在终端中输入python ImageNet.py即可运行，若要使用分布式方法，需要在终端中输入以下命令：
# python -m torch.distributed.run --nproc_per_node=2 ImageNet.py
...
class CBNModule(nn.Module):#本模型中使用的经修改的CBN
    def __init__(self, num_features, eps=1e-5, momentum=0.1, affine=True,
        ...
        # 注册 self.ones 作为 buffer，使其能随模型移动到正确设备
        self.register_buffer('ones', torch.ones(self.num_features)) 
    ...
    def reset_parameters(self):# 初始化参数
        if self.track_running_stats:
            if self.running_mean is not None:# 确保 self.running_mean 和 self.running_var 可导
                self.running_mean.zero_()
            if self.running_var is not None:
                self.running_var.fill_(1)
        if self.affine:# 确保 self.weight 和 self.bias 可导
            if self.weight is not None:
                self.weight.data.uniform_()
            if self.bias is not None:
                self.bias.data.zero_()

    def _check_input_dim(self, input):# 确保输入是4D张量
        if input.dim() != 4:
            raise ValueError('expected 4D input (got {}D input)'.format(input.dim()))

    def _update_buffer_num(self):
        if self.two_stage:# 双阶段更新
            if self.iter_count > self.burnin:
                self.buffer_num = self.max_buffer_num
            else:
                self.buffer_num = 0
        else:
            if self.burnin == 0: #避免除以0，如果 burnin 为 0，则直接设为 max_buffer_num
                self.buffer_num = self.max_buffer_num
            else:
                self.buffer_num = int(self.max_buffer_num * min(self.iter_count / self.burnin, 1.0))

    def forward(self, input):#修改: forward 方法签名不再包含 weight 参数
        self._check_input_dim(input) # 确保 self.ones 和 input 在同一个设备上
        # self.ones 已经作为 buffer 注册，会自动随模型移动，无需再次 .to()
        y = input.transpose(0, 1).contiguous() # C, N, H, W -> N, C, H, W -> C, N, H, W , BN 层的输入通常是 (N, C, H, W)
        return_shape = input.shape 
        y = y.view(input.size(1), -1) 
        #`transpose(0,1)` 效果是 (N,C,H,W) -> (C,N,H,W)
        # 然后 `view(input.size(1), -1)` 会把 (C, N*H*W)
        #  mu 和 sigma2 是 (C,) 维度的，表示每个通道的统计量。
        ...
        if self.buffer_num > 0 and self.training and input.requires_grad:
            ...
            if self.affine and self.weight.requires_grad: # 确保 self.weight 是可导的，并且在正确的设备上
                try:
                    # 使用 allow_unused=True
                    dmudw = torch.autograd.grad(cur_mu, self.weight, self.ones, retain_graph=True, allow_unused=True)[0]
                    dmeanx2dw = torch.autograd.grad(cur_meanx2, self.weight, self.ones, retain_graph=True, allow_unused=True)[0]

                except RuntimeError as e:
                    print(f"!!! WARNING: torch.autograd.grad failed for dmudw/dmeanx2dw: {e}. Setting to zero.")
                    dmudw = None 
                    dmeanx2dw = None
            
            # 处理 dmudw 和 dmeanx2dw 可能为 None 的情况
            if dmudw is None:
                dmudw = torch.zeros_like(self.weight).detach()
            if dmeanx2dw is None:
                dmeanx2dw = torch.zeros_like(self.weight).detach()

            #  裁剪 dmudw 和 dmeanx2dw 本身，防止其过大导致后续计算溢出 
            if not (torch.all(dmudw == 0) and torch.all(dmeanx2dw == 0)): # 只有当它们不是全部为0时才裁剪，以避免不必要的计算和日志
                dmudw = torch.clamp(dmudw, min=-1e3, max=1e3) 
                dmeanx2dw = torch.clamp(dmeanx2dw, min=-1e3, max=1e3) 

            # 检查 dmudw/dmeanx2dw 中的 NaN/Inf，如果它们是在 grad 内部产生的
            if dmudw.isnan().any() or dmudw.isinf().any():
                print(f"!!! DEBUG: dmudw is NaN/Inf at iter {self.iter_count}. Min/Max: {dmudw.min().item() if dmudw.numel() > 0 else 'N/A'}/{dmudw.max().item() if dmudw.numel() > 0 else 'N/A'}")
                dmudw[dmudw.isnan()] = 0.0 # 尝试将 NaN/Inf 梯度设置为 0.0，避免梯度爆炸
                dmudw[dmudw.isinf()] = 0.0
            if dmeanx2dw.isnan().any() or dmeanx2dw.isinf().any():
                print(f"!!! DEBUG: dmeanx2dw is NaN/Inf at iter {self.iter_count}. Min/Max: {dmeanx2dw.min().item() if dmeanx2dw.numel() > 0 else 'N/A'}/{dmeanx2dw.max().item() if dmeanx2dw.numel() > 0 else 'N/A'}")
            ...
            for tmp_mu, tmp_d_mu, tmp_meanx2, tmp_d_meanx2, tmp_w in zip(
                list(self.pre_mu), list(self.pre_dmudw), list(self.pre_meanx2), list(self.pre_dmeanx2dw), list(self.pre_weight)
            ):
                # 确保所有操作在同一设备上 (这些张量来自历史缓冲区，可能不在当前设备)
                tmp_mu_on_device = tmp_mu.to(self.weight.device)
                tmp_d_mu_on_device = tmp_d_mu.to(self.weight.device)
                tmp_meanx2_on_device = tmp_meanx2.to(self.weight.device)
                ...
                #  裁剪 delta_mu 和 delta_meanx2，防止其过大导致溢出 
                delta_mu = torch.clamp(delta_mu, min=-1e1, max=1e1) 
                delta_meanx2 = torch.clamp(delta_meanx2, min=-1e1, max=1e1) 
                # 确保 tmp_mu 和 tmp_meanx2 也在正确的设备上
                mu_all.append(tmp_mu_on_device + delta_mu)
                meanx2_all.append(tmp_meanx2_on_device + delta_meanx2)
            ...
            sigma2_all = torch.clamp(sigma2_all, min=0.0) 

            count = (sigma2_all >= 0).sum(dim=0).float() # 计算有效统计量的数量
            count = torch.max(count, torch.tensor(1.0, device=count.device))# 至少为 1，防止除以零
            ...
            # 更新历史缓冲区
            self.pre_mu = [cur_mu.detach()] + self.pre_mu[:(self.max_buffer_num - 1)]
            self.pre_meanx2 = [cur_meanx2.detach()] + self.pre_meanx2[:(self.max_buffer_num - 1)]
            self.pre_dmudw = [dmudw.detach()] + self.pre_dmudw[:(self.max_buffer_num - 1)]
            self.pre_dmeanx2dw = [dmeanx2dw.detach()] + self.pre_dmeanx2dw[:(self.max_buffer_num - 1)]

            tmp_weight = self.weight.data.clone() 
            self.pre_weight = [tmp_weight.detach()] + self.pre_weight[:(self.max_buffer_num - 1)]

        else: # 当 buffer_num == 0 或者模型不在训练模式 (self.training is False) 或者 input.requires_grad 为 False 时
            # 退化为标准 BN 或使用 running stats
            # 无论如何，都需要计算当前批次的统计量，因为它们可能是 fallback
            x = y 
            cur_mu = x.mean(dim=1)
            cur_meanx2 = torch.pow(x, 2).mean(dim=1)
            cur_sigma2 = cur_meanx2 - torch.pow(cur_mu, 2)
            cur_sigma2 = torch.clamp(cur_sigma2, min=0.0) # 钳制当前批次的方差

            if self.track_running_stats and (not self.training or self.FROZEN):
                # 如果追踪运行统计量 并且 (处于评估模式 或者 处于冻结模式)，则使用运行统计量
                # 在这种情况下，self.running_mean 和 self.running_var 保证是张量
                mu = self.running_mean
                sigma2 = self.running_var
                sigma2 = torch.clamp(sigma2, min=self.eps) # 确保运行方差非负且不小于eps
            else:
                # 这种情况包含两种情况：
                # 1. self.track_running_stats 为 False：始终使用当前批次统计量 (无论训练或评估)
                # 2. self.training 为 True 且 not self.FROZEN (并且处于此 'else' 分支，即 buffer_num == 0)：
                #    这是烧入期 (burn-in)，此时也使用当前批次统计量
                mu = cur_mu
                sigma2 = cur_sigma2
            
            # 如果追踪运行统计量 并且 处于训练模式 (并且未冻结)，则更新运行统计量
            if self.track_running_stats and self.training and not self.FROZEN:
                with torch.no_grad():
        ...
        #  规范化步骤和最终仿射变换  
        # 此时 mu 和 sigma2 保证是张量
        std = torch.sqrt(sigma2 + self.eps) # 确保方差为正且不为零，防止除以零
        std = torch.where(std == 0, torch.full_like(std, self.eps), std) # 再次确保 std 不为零
        normalized_y = (y - mu.view(-1, 1)) / std.view(-1, 1)# 进行规范化处理
        if self.affine:# 执行缩放和平移
            output = self.weight.view(-1, 1) * normalized_y + self.bias.view(-1, 1)
        else:
            output = normalized_y
        original_shape = input.shape 
        output = output.view(input.size(1), input.size(0), *input.shape[2:])
...
#3.数据加载器与模型定义
def get_voc_dataloader(batch_size, voc_root='VOC2007', distributed=True):# 数据加载器
    # 路径检查
    ...
    train_dataset = VOCDetection(
        ...
        year='2007',
        image_set='trainval',
        download=False,#当前目录不包括VOC2007数据集，会自动下载
...
def get_retinanet_model(norm_type='SyncBN', num_classes=21): # 模型定义
    """
    获取配置好的RetinaNet模型。
    Args:
        norm_type (str): 归一化层类型 ('SyncBN', 'CBN', 'None')。
        num_classes (int): 数据集类别数，包含背景类。本论文选择的数据集为VOC2007，
            包含20个物体类别和一个背景类，因此num_classes=21。
    Returns:
        torch.nn.Module: 配置好的RetinaNet模型。
    """
    weights = ResNet50_Weights.DEFAULT #加载预训练权重
    backbone = resnet50(weights=weights, norm_layer=nn.BatchNorm2d) # 加载ResNet50骨干网络
    hooks = []
    # 替换BatchNorm层
    if norm_type == 'CBN':# 替换BatchNorm层为CBN
        print("正在将ResNet50骨干网络中的BatchNorm层替换为CBNModule...")
        def replace_bn_with_cbn(module):
            for name, child in module.named_children(): #遍历所有子模块
                if isinstance(child, nn.BatchNorm2d): # 检查是否是BatchNorm2d
                    # 复制BatchNorm的权重和偏置到CBN，但跳过num_batches_tracked，
                    # 因为CBNModule没有这个属性。
                    cbn_layer = CBNModule(child.num_features)# 创建CBNModule实例
                    cbn_layer.weight.data.copy_(child.weight.data) # 复制权重
                    cbn_layer.bias.data.copy_(child.bias.data) # 复制偏置
                    setattr(module, name, cbn_layer) # 替换BatchNorm层为CBNModule
                else:
                    replace_bn_with_cbn(child) # 递归处理子模块
        replace_bn_with_cbn(backbone) # 递归替换
    elif norm_type == 'SyncBN': # 替换BatchNorm层为SyncBN
        print("正在将ResNet50骨干网络中的BatchNorm层替换为SyncBatchNorm...")
        # 对于SyncBN，直接通过 norm_layer 传递给 FPN extractor
        # _resnet_fpn_extractor 会处理 backbone 的 BatchNorm 替换
        # 如果模型已经在 DDP 中，SyncBN 会在 DDP 包装时自动处理
        pass # 在 _resnet_fpn_extractor 中处理
    elif norm_type == 'None': # 不使用任何规范化层
        print("骨干网络不使用任何规范化层。")
        def remove_bn(module):
            for name, child in module.named_children():
                if isinstance(child, (nn.BatchNorm2d, CBNModule, nn.SyncBatchNorm)):# 移除所有BN层
                    setattr(module, name, nn.Identity()) # 替换为Identity层
                else:
                    remove_bn(child) # 递归处理子模块
        remove_bn(backbone)
    # 构造带FPN的骨干网络
    # 对于SyncBN，我们在这里指定norm_layer
    if norm_type == 'SyncBN':# 对于SyncBN，我们在这里指定norm_layer
        backbone_with_fpn = _resnet_fpn_extractor(backbone, extra_blocks=None, norm_layer=nn.SyncBatchNorm, trainable_layers=3)
    else:
        # 对于CBN或None，_resnet_fpn_extractor 会使用默认的BatchNorm2d，
        # 但我们已经在上面手动替换了backbone中的BN层，
        # FPN内部的额外conv层（layer_blocks, inner_blocks）的norm_layer需要单独处理。
        # _resnet_fpn_extractor 默认的 FPN 会使用 BatchNorm2d，所以 FPN 自己的 BN 也需要继续替换。
        # 因此，我们需要处理CBN/None的情况。
        # 鉴于 torchvision.models.detection.retinanet.RetinaNet 内部已经处理了 FPN 的创建，
        # 我们在这里主要处理 backbone 本身的替换，FPN 内部的 conv 层的 norm_layer
        # 如果不是通过 _resnet_fpn_extractor 的 norm_layer 参数统一控制，
        # 那么就需要在 FPN 创建后遍历其模块进行替换。
        # 为了简化，我们假设 _resnet_fpn_extractor 的默认行为对 CBN/None 是可接受的，
        backbone_with_fpn = _resnet_fpn_extractor(backbone, extra_blocks=None,trainable_layers=3)
        # 对于CBN或None，进一步替换FPN内部的BatchNorm层
        if norm_type == 'CBN':
            print("正在将FPN内部的BatchNorm层替换为CBNModule...")
            def replace_fpn_bn_with_cbn(module):# 递归替换FPN内部的BN层
                for name, child in module.named_children():
                    if isinstance(child, nn.BatchNorm2d):# 检查是否是BatchNorm2d，如果是则替换为CBNModule
                        cbn_layer = CBNModule(child.num_features)
                        cbn_layer.weight.data.copy_(child.weight.data)
                        cbn_layer.bias.data.copy_(child.bias.data)
                        setattr(module, name, cbn_layer)
                    else:
                        replace_fpn_bn_with_cbn(child)
            replace_fpn_bn_with_cbn(backbone_with_fpn.fpn) # 仅替换FPN部分
        elif norm_type == 'None':
            print("正在移除FPN内部的规范化层。")
            def remove_fpn_bn(module):
                for name, child in module.named_children():
                    if isinstance(child, (nn.BatchNorm2d, CBNModule, nn.SyncBatchNorm)):# 移除所有BN层
                        setattr(module, name, nn.Identity())
                    else:
                        remove_fpn_bn(child)
            remove_fpn_bn(backbone_with_fpn.fpn) # 仅替换FPN部分
    # 设置所有参数都可训练
    for p in backbone_with_fpn.parameters():
        p.requires_grad = True
    # RetinaNet模型的anchor generator
    anchor_sizes = tuple((x, ) for x in [32, 64, 128, 256, 512])# 每个特征图的anchor大小
    aspect_ratios = ((0.5, 1.0, 2.0),) * len(anchor_sizes)# 每个特征图的aspect ratios
    anchor_generator = AnchorGenerator(anchor_sizes, aspect_ratios)# 创建anchor generator
    # 获取默认的RetinaNet模型，它会自动使用FPN作为骨干
    # 替换预训练模型的分类头和回归头以匹配新的类别数
    model = RetinaNet(
        backbone_with_fpn,
        num_classes=num_classes,
        anchor_generator=anchor_generator
    )
    def fpn_output_hook(module,input, output): # 检查FPN输出是否追踪梯度
        if not module.training: # module.training 为 False 表示 eval 模式
            pass # 而如果模型处于评估模式，且你在评估阶段，就不需要检查梯度
        else: # 训练模式
            for k, v in output.items():#遍历FPN输出的每个特征图
                if v.requires_grad:
                    pass
                else:
                    print(f"    requires_grad: {v.requires_grad} (!!! 警告: FPN输出不追踪梯度)")
                if torch.isnan(v).any() or torch.isinf(v).any():
                    print(f"    !!! 警告: 特征图 '{k}' 包含 NaN 或 Inf 值!")
    # 注册 hook 到 FPN 模块
    # backbone_with_fpn 内部包含了 FPN
    # 我们可以尝试注册到 model.backbone.fpn 上。
    if hasattr(model, 'backbone') and hasattr(model.backbone, 'fpn'):# 检查模型是否包含FPN
        model.backbone.fpn.register_forward_hook(fpn_output_hook)
    else:
        print("警告：无法在 model.backbone.fpn 上注册 forward hook。")
    print("正在确保FPN参数可训练...")
    for name, p in model.backbone.fpn.named_parameters():
        if p.requires_grad is False:
            p.requires_grad = True # 显式设置为 True
...
def train_model(model, train_loader, val_loader, optimizer, scheduler, device, num_epochs,
                ...
                model_save_path, rank=0, eval_interval=5): 
    """
    训练模型主循环
    Args:
...
def main():
    ...
    batch_sizes = [8] 
    num_epochs = 100 # 训练轮数 
    initial_learning_rate = 0.00005 #初始学习率
    ...
    for batch_size in batch_sizes:#便于训练不同batch_size
        ...
        for model_name, info in models_info.items():
            for norm_type in norm_types:
                ...
                # 确保路径一致性
                log_base_dir = os.path.join('CBN_Image', model_name, norm_type, f'bs_{batch_size}_lr_{initial_learning_rate}_{current_time_str}')
